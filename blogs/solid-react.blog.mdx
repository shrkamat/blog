...
function Charmander({ isPoweredUp }) {
  return (
    <div>
      ...
      <img src={`charmander${isPoweredUp ? '_powered' : ''}.png`} alt="Charmander" />
    </div>
  );
}
...
This approach ensures that each principle is clearly explained with relevant and practical examples from the Pokémon game app, providing a comprehensive and engaging guide to applying SOLID principles in React development.
